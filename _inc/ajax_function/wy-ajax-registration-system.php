<?php
add_action('wp_ajax_nopriv_wy_ajax_registration_system', 'wy_ajax_registration_system');

function wy_ajax_registration_system() {
    ...
    $phone_number = sanitize_text_field($_POST['phone_number_user']);
    ValidateParameter::validate_registration_system($full_name, $email, $password);
    $user_data = [
    ...
    if ( ! is_wp_error( $insert_user_in ) ) {
         if (!get_user_meta($insert_user_in, '_wy_user_phone_number', true) ) {
             add_user_meta($insert_user_in,'_wy_user_phone_number',$phone_number);
             ...
             $bodyId =get_option( '_wy_sms_website')['sms_data']['welcome'];
             Sms::setter(Helper::wy_create_display_name($full_name)['display_name'],$phone_number,$bodyId);
         }else{
             wp_send_json( ['error'=>true,'message'=>'قبلا با این شماره موبایل ثبت نام کرده اید!'],403);
         }
    } else {
